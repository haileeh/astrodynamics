function continuation(X0_1,X0_2,mu)
	...
	if abs(x02-x01) > abs(z02-z01) # x is fixed state
	    ...
	    X0_new = [x02+eps; 0.0; z0_new; 0.0; ydot0_new; 0.0];
	else # z is fixed state
	    x0_new = (x02 - x01)/(z02-z01)*eps + x02;
	    ydot0_new = (ydot02 - ydot01)/(z02-z01)*eps + ydot02;
	    X0_new = [x0_new; 0.0; z02+eps; 0.0; ydot0_new; 0.0];
	end

	# call differential corrector
	(X0_new,t_half_new)=differentialControl_zFixed(mu,X0_new,3.0);

	return X0_new, t_half_new
end